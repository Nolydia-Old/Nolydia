...
public class LocaleArgument implements Argument<Locale> {
    ...
    public InternalizationMessage getName() {
        return new InternalizationMessage("locale.argument");
    }
    ...
    public Locale get(CommandSender sender, String argument) {
        ...
        if (optionalLocale.isEmpty()) {
            throw new ArgumentException(new InternalizationMessage("no_matching_locale", new InternalizationArgument("locale", argument)));
        }

        Locale locale = optionalLocale.get();

        if (locale == sender.getLocale()) {
            throw new ArgumentException(new InternalizationMessage("has_already_locale", new InternalizationArgument("locale", locale)));
        }
